...
/**
 * Characterization test — EPIC CDX-RAG-1 : mirror of codex `CodexVectorStoreTest`.
 *
 * Verbatim migration of `codex.store.CodexVectorStore` into `codebase.store`
 * as `RagVectorStore` (Brooklyn → Queens, N2 → N1). The connection contract,
 * embedding pipeline (ONNX AllMiniLmL6V2 384 dim) and search API are
 * identical — only the package and class name change. Tables `codex_documents`
 * / `codex_chunks` stay unchanged (Loi de l'Économie d'Encre).
 */
class RagVectorStoreTest {
    ...
    @Test
    fun `searchBlocking is a valid method`() {
        val store = RagVectorStore()
