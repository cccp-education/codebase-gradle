...
Feature: EPIC 6 Quality Gate — Validation boucle qualité ONNX (checks déterministes MVP)
  ...
  Scenario: Clean CODING output passes all quality checks
    When I evaluate CODING output "class UserRepository { fun findByName(name: String): User = entityManager.createQuery(...).singleResult }"
  ...
  Scenario: PII residual in output triggers FAIL verdict
    When I evaluate CODING output "token=ghp_1234567890abcdefghijkl class Broken {}"
  ...
  Scenario: Off-topic output triggers FAIL for wrong domain
    When I evaluate CONTENT output "@SpringBootApplication class App"
    Then the quality gate fails with verdict "FAIL"

  Scenario: Mixed issues produce FAIL and detailed assessment
    When I evaluate CODING output "La recette du gâteau. email=admin@test.com password=secret"
    Then the quality gate fails with verdict "FAIL"
    And the assessment contains at least 2 failing checks

  Scenario: Feedback message includes all failing check details
    When I evaluate CODING output "ghp_token_0123456789abcdefghijkl class Broken val x = 42"
  ...
  Scenario: Sentiment-negative output triggers NEEDS_FIX or FAIL
    When I evaluate CODING output "C'est nul, horrible, un désastre total, une erreur grave."
    Then the quality gate verdict is not "PASS"

  Scenario: Empty output passes all checks with max score
    When I evaluate CODING output ""
    Then the quality gate passes
